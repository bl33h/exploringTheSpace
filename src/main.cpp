...
bool init() {
    ...
    window = SDL_CreateWindow("Exploring The Space With bl33h", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
    if (!window) {
        SDL_Quit();
        return false;
    }
...
int main(int argc, char* argv[]) {
    ...
    std::string spaceshipFilePath = "src/objects/spaceship.obj";
    ...
    Camera camera(glm::vec3(0.0f, 0.0f, 17.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_KEYDOWN) {
            switch (event.key.keysym.sym) {
                case SDLK_LEFT:
                    ...
                    oy = oy + -0.5;
                    rotate = glm::rotate(glm::mat4(1.1f), glm::radians(5.0f), glm::vec3(0.0f, 1.0f, 0.0f));
                ...
                case SDLK_RIGHT:
                    ...
                    break;
                case SDLK_UP:
                    camera.moveForward(1.1f);
                    break;
                case SDLK_DOWN:
                    camera.moveBackward(1.1f);
                    break;
                }
            }
        }
        ...
        planetSpeed += 0.8;
        glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), glm::radians(planetSpeed), rotationAxis);
        ...
        // random
        float RandomOrbitRadius = 3.0f;
        float RandomRotationAngle = glm::radians(planetSpeed);
        ...
        // places the destroyed spaceship
        ...
        currentshaderType = shaderType::destroyedSpaceship;
        render(vertexBufferSpace, spaceshipUniforms);

        // pluton
        float PlutonOrbitRadius = -3.0f;
        float PlutonRotationAngle = glm::radians(planetSpeed);
        ...
        float VenusRotationAngle = glm::radians(planetSpeed);
        ...
        float earthRotationAngle = glm::radians(planetSpeed);
        ...
        if (currentshaderType == shaderType::Earth) {
            ...
            float moonRotationAngle = glm::radians(planetSpeed * -1.5f);
        ...
        // neptune
        float NeptuneOrbitRadius = 8.0f;
        float NeptuneRotationAngle = glm::radians(planetSpeed);
